...
1. [**Sensing**](components/sensing.md) captures and preprocesses raw sensor data (LiDAR, camera, IMU).
2. [**Perception**](components/perception.md) detects and tracks objects, traffic lights, and drivable space.
3. [**Mapping**](components/mapping.md) serves high-definition map data that the rest of the stack consumes.
4. [**Localization**](components/localization.md) determines the vehicle's exact position on the map.
5. [**Planning**](components/planning.md) computes a safe, feasible trajectory to the goal.
6. [**Control**](components/control.md) converts that trajectory into throttle, brake, and steering commands.
